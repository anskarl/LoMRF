...
a well-known AI test case of non-monotonic temporal reasoning which is proposed by Steve Hanks and Drew McDermott[^1]. 
...
happen (e.g., the hunter loads the gun), causing related situations hold or not (e.g., the gun is loaded).
...
The contents of the `theory.mln` file is can be the following:
  ...
  *month* (i.e., representing the month number), i.e., `month = bar(time)`.
  ...
  5. A special case of formulas are the definite clauses, with which we define declarations of rules. The definite 
  clauses, are processed by the LoMRF and automatically translated to equivalent formulas. Their syntax has simpler 
  from formulas and are ideal for defining domain-specific knowledge.
...
to define that subset symbols in the theory.  

The domain types that we use in our example are belong to the domains of *time*, *event* and *fluent*. The values where 
...
terminated at `t`, will not hold at the next time-point `t++`. The third formula expresses that the fluent `f` continues 
to hold at the next time-point when it is not terminated at the current time-point `t`. Similarly, the last formula 
expresses that the fluent continues to not hold at the next time-point when the it is not initiated at the current
time-point.
...
knowledge for our running example. For simplicity we will use the definite clause notation in that part. All definite 
...
In LoMRF, clauses can be either soft or hard-constrained. Similarly with the formula syntax, hard-constrained clauses
end with a dot symbol, while soft-constrained are prepended with a weight value. 
...
In order to perform inference, we have to define the following:
  * The type of the inference. In LoMRF two inference can be performed: 
    1. Marginal inference: for computing the probabilities for all possible instantiations of query predicate of being true, given the evidence.
    2. Maximum a-posteriori inference (MAP): for computing the truth value (0 = False and 1 = True) for all possible instantiations that together represent to the most probable state.
