...
import lomrf.mln.model.mrf.{Constraint, MRF}
 ...
 */
final class ILP2(mrf: MRF) extends LPModel(LPSolverLib.lp_solve) with Logging {

  def infer(out: PrintStream = System.out) {

    // Hash map containing pairs of unique literal values to LD variables
    val cacheLiterals = new mutable.HashMap[Int, LPFloatVar]()
    val cacheClauses = new mutable.HashMap[Int, LPFloatVar]()
