 ...
 */
private final class GroundingMaster(mln: MLN, latch: CountDownLatch, noNegWeights: Boolean, experimentalGrounder: Boolean = false) extends Actor with Logging {
  ...
  private val clauseGroundingWorkers: Array[ActorRef] = {
    ...
    yield context.actorOf(Props(new GroundingWorker(mln, cliqueRegisters, noNegWeights, experimentalGrounder)), name = "grounding_worker-" + i)
      )(breakOut)
  }
